import { clients, newClient } from "./apiFaker.js";
...
// o caractere dado.
function findNameByFirstLetter(char) {
...
function listByClientIndexName() {
...
function findAmountForAChar(char) {
  var count = 0;
...
function findFirstNameByFirstLetter(char) {
  const filteredNames = clients.filter(client => {
    return client.name.charAt(0) === char.toUpperCase();
  });
...
// mais de 15 compras.
function moreThenFifteenClientPurchase() {  
  const datas = clients.filter(client => {
...
function addNewClientAtStart(newClient) {
